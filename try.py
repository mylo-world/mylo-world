...
#         Cloud4.image = Cloud4Path
#         Cloud4.place(relx=0.89, rely=0.12, anchor='center')
#         animate_label(Cloud4, frame1, x=20, y=75, x_speed=1, y_bounds=(20, 45))
...
#                 index = 0  # Track index across all words
#                 for word in self.chosenwords.split(' '):  # Process each word separately
#                     for char in word:  # Iterate through characters in the word
#                         if key.lower() == char:
#                             self.win_count += 1
#                             play_correct_sound()
#                             self.Display[index].config(text=key.upper())  # Update the correct underscore
#                         index += 1  # Move to the next label in Display
...
#                 if key in self.buttons:
#                     self.buttons[key].destroy()
#                 if self.count <= 6:  # Assuming there are 6 stages of Vudiddy images
#                     Vudiddy = Image.open(relative_path(f"Assets/Vudiddy{self.count}.png")).convert("RGBA")
#                     Vudiddy = Vudiddy.resize((350, 350), Image.Resampling.LANCZOS)
#                     VudiddyTkImage = ImageTk.PhotoImage(Vudiddy)
...
#                     self.VudiddyPicture.image = VudiddyTkImage  # Keep reference alive

#                 self.start_time=time.time()
#                 self.timer_label=tk.Label (frame1, text="Time=0", font=("Helvetica",20),bg='grey',fg='white')
...
#                 if self.win_count == len(self.chosenwords):
#                     for widget in self.master.winfo_children().replace(" ",""):
#                         widget.destroy()
#                     WinWindow(self.master, self.LoadDefs, self.chosenwords)
...
#     #     if.self.win_count == len(self.choosenwords):
#     #         for widget in self.master.winfo_children():
#     #             widget.destroy()
#     #         total_time=int(time.time()-self.start_time)
#     #         WinWindow(self.master,self.LoadDefs, self.choosenwords, total_time)
...
#         DefinitionLabel = create_text_image(
...
#             size=(1000, 150)
#         )
#         Definitions = tk.Label(frame1, image=DefinitionLabel, borderwidth=0)
#         Definitions.image = DefinitionLabel
#         Definitions.place(relx=0.5, rely=0.4, anchor='center')
...
class LoseWindow:
    def __init__(self, master, LoadDefs, chosenwords):
        ...
        SetBackground(frame1, relative_path("Assets/frame6.png"))
        ...
        canvas = tk.Canvas(frame1, width=800, height=150, borderwidth=0, highlightthickness=0, bg="#12022b") 
        scrollbar = ttk.Scrollbar(frame1, orient="vertical", command=canvas.yview) 
        style=ttk.Style()
        style.configure("Vertical.TScrollbar", background="#12022b", troughcolor="#12022b", arrowcolor="#12022b") 
        ...
        Definitions = tk.Label(scrollable_frame, image=DefinitionLabel, borderwidth=0) 
        Definitions.image = DefinitionLabel 
        Definitions.pack() 
        
        canvas.place(relx=0.5, rely=0.4, anchor='center') 
        scrollbar.place(relx=0.95, rely=0.4, anchor='center', height=150)

        # # Create a frame to hold the Text widget and scrollbar 
        # text_frame = tk.Frame(frame1) 
        # text_frame.place(relx=0.5, rely=0.45, anchor='center', width=800, height=200)
        ...
        #     font_path=("Rocabe", 15), bg="#12022b", fg="white", borderwidth=0 
        # ) 
        # text_box.insert(tk.END, f"The definition of {chosenwords}\n{LoadDefs[chosenwords]}") 
        ...
        self.button_again.image = PlayAgainButton
        self.button_again.place(relx=0.545, rely=0.8, anchor='center')
        ...
        self.button_menu.image = MainMenuButton
        self.button_menu.place(relx=0.7, rely=0.8, anchor='center')
        self.button_again.text="Play Again",
